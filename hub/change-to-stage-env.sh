#!/usr/bin/env bash
# configure-stage-env.sh — run as root; resilient to failures.
...
# Set EnvironmentType to "\"Stage\"" (escaped quotes around Stage)
try docker exec redis redis-cli set EnvironmentType "\"Stage\""
