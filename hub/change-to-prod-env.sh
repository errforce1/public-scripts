#!/usr/bin/env bash
# configure-prod-env.sh — run as root; resilient to failures.
...
# Set EnvironmentType to "\"Prod\"" (escaped quotes around Prod)
try docker exec redis redis-cli set EnvironmentType "\"Prod\""
